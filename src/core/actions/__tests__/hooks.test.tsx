...
type SendFeedbackResponse = Awaited<ReturnType<typeof actions.sendFeedback>>;
type TranslateResponse = Awaited<ReturnType<typeof actions.translate>>;

describe("core/actions/hooks", () => {
	it("useSendFeedback should return true on success", async () => {
		vi.spyOn(actions, "sendFeedback").mockResolvedValue({ success: true } as unknown as SendFeedbackResponse);
	...
	it("useSendFeedback should throw on error", async () => {
		vi.spyOn(actions, "sendFeedback").mockResolvedValue({
			success: false,
			error: "fail",
		} as unknown as SendFeedbackResponse);
		const { result } = renderHook(() => useSendFeedback());
		await expect(
	...
	it("useTranslateRequest should call onMutate/onSettled passthrough", async () => {
		vi.spyOn(actions, "translate").mockResolvedValue({ success: true, data: "GLOSS" } as unknown as TranslateResponse);
